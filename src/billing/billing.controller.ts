...
export class BillingController {
        ...
        try {
          if (customer.type === "pppoe") {
            const secrets = await client.run("/ppp/secret/print", {
              "?name": customer.mikrotikUser
            });
            if (secrets[0]?.[".id"]) {
              await client.run("/ppp/secret/disable", {
                ".id": secrets[0][".id"]
              });
              disabled++;
            }
            ...
            if (users[0]?.[".id"]) {
              await client.run("/ip/hotspot/user/disable", {
                ".id": users[0][".id"]
              });
              disabled++;
            }
  ...
  async reEnable(@Param("session") session: string, @Param("id") id: string) {
    ...
    try {
      if (cust.type === "pppoe") {
        const s = await client.run("/ppp/secret/print", {
          "?name": cust.mikrotikUser
        });
        if (s[0]?.[".id"])
          await client.run("/ppp/secret/enable", { ".id": s[0][".id"] });
      } else {
        const u = await client.run("/ip/hotspot/user/print", {
          "?name": cust.mikrotikUser
        });
        if (u[0]?.[".id"])
  ...
  async sendReminder(
    ...
    if (!this.telegramSvc)
      return { error: "Telegram service belum dikonfigurasi" };
  ...
  @Post(":session/settlements/submit")
  submitSettlement(
    @Param("session") session: string,
    @Body() body: { collectorId: string; collectorName: string; amount: number }
  ) {
    return this.billingSvc.submitSettlement(
      session,
      body.collectorId,
      body.collectorName,
      body.amount
  ...
  getSettlementSummary(
    ...
    return {
      success: true,
      data: {
        unsetteled: collector?.unsettledCash || 0, // Pastikan typo 'unsetteled' sama dengan di Mobile UI
