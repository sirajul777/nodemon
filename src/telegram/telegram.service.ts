...
import {
  VoucherTypeService,
  VoucherType
} from "../voucher-types/voucher-type.service";
...
const CONFIG_FILE = path.join(process.cwd(), "telegram.json");
...
export class TelegramService implements OnModuleInit {
  ...
  private getFilePath(base: string, sessionId?: string): string {
    ...
    const filename = sessionId
      ? `${base.replace(".json", "")}-${sessionId}.json`
      : base;
  ...
  setServices(
    mikrotik: any,
    config: any,
    vtService?: VoucherTypeService,
    resellerSvc?: BotResellerService,
    resellerTgSvc?: BotResellerTelegramService
  ) {
  ...
  onModuleInit() {
    setTimeout(() => {
      ...
      for (const cfg of configs) {
        this.startPolling(cfg.id);
        this.scheduleDailyReport(cfg.id);
      }
    }, 3000);
  ...
  getAllConfigs(): TelegramConfig[] {
    try {
      if (fs.existsSync(CONFIG_FILE)) {
        ...
        // Support format lama (single object) dan baru (array)
        if (Array.isArray(data)) return data;
        if (data.token) return [{ ...data, id: data.sessionId || "default" }];
      }
    } catch {}
    return [];
  }

  getConfig(id?: string): TelegramConfig | null {
    const all = this.getAllConfigs();
    if (!id) return all[0] || null; // backward compat
  ...
  saveConfig(config: TelegramConfig): void {
    ...
    // Restart polling untuk bot ini saja
    this.stopPolling(config.id);
    setTimeout(() => this.startPolling(config.id), 1000);
    this.scheduleDailyReport(config.id);
  }

  deleteConfig(id: string): void {
    const all = this.getAllConfigs().filter((c) => c.id !== id);
    fs.writeFileSync(CONFIG_FILE, JSON.stringify(all, null, 2));
    this.stopPolling(id);
  }
  ...
  private readonly TOPUP_FILE = path.join(
    process.cwd(),
    "data",
    "topup-requests.json"
  );
  ...
  async sendMessage(
    ...
    // Cari config yang punya chatId ini, atau pakai configId
    let cfg: TelegramConfig | null = null;
    if (configId) {
      cfg = this.getConfig(configId);
    } else {
      // Cari bot yang handle chatId ini
      const all = this.getAllConfigs();
      cfg =
        all.find(
          (c) => c.chatId === chatId || c.allowedUsers?.includes(chatId)
        ) ||
        all[0] ||
        null;
    }
    ...
    try {
      return await this.apiCall(cfg.token, "sendMessage", {
        chat_id: chatId,
        text,
        parse_mode: "HTML",
        ...extra
  ...
  async sendTest(
    ...
    try {
      ...
      if (res.ok) {
        this.addLog("SYSTEM", `Test sent to ${chatId}`);
        return { success: true };
      }
  ...
  private async startPolling(configId: string): Promise<void> {
    ...
    state.polling = true;
    this.logger.log(`Bot [${configId}] polling started`);

    while (state.polling) {
      try {
        const cfg2 = this.getConfig(configId);
        if (!cfg2?.token || !cfg2.botEnabled) {
          state.polling = false;
          break;
        }
        ...
        if (res.ok && res.result?.length) {
          for (const update of res.result) {
            ...
            if (update.message) await this.handleMessage(update.message, cfg2);
            else if (update.callback_query)
              await this.handleCallback(update.callback_query, cfg2);
          }
        }
      } catch (e) {
        if (state.polling) {
          this.logger.warn(`Bot [${configId}] polling error: ${e}`);
          await new Promise((r) => setTimeout(r, 5000));
        }
      }
    }

    this.logger.log(`Bot [${configId}] polling stopped`);
  }

  private stopPolling(configId: string): void {
    const state = this.botStates.get(configId);
    if (state) {
      state.polling = false;
      if (state.dailyTimer) {
  ...
  // Stop semua bot
  stopAllPolling(): void {
    for (const [id] of this.botStates) {
      this.stopPolling(id);
    }
  }
  ...
  private async handleMessage(msg: any, cfg: TelegramConfig): Promise<void> {
    ...
    switch (command) {
      case "/start":
      case "/help":
        await this.handleHelp(chatId, isAdmin);
        break;
      case "/beli":
        await this.handleBeliMenu(chatId, userId, username, args, cfg, "beli");
        break;
      case "/generate":
        await this.handleBeliMenu(
          chatId,
          userId,
          username,
          args,
      ...
      case "/profil":
      case "/profile":
        await this.handleProfil(chatId, cfg);
        break;
      case "/cek":
        await this.handleCek(chatId, args, cfg);
        break;
      case "/status":
        if (isAdmin) await this.handleStatus(chatId, cfg);
        break;
      case "/aktif":
        if (isAdmin) await this.handleAktif(chatId, cfg);
        break;
      case "/rekap":
      case "/today":
        if (isAdmin) await this.handleRekap(chatId, "today", cfg);
        break;
      case "/bulan":
      case "/month":
        if (isAdmin) await this.handleRekap(chatId, "month", cfg);
        break;
      case "/pppoe":
        if (isAdmin) await this.handlePppoe(chatId, cfg);
        break;
      case "/hapus":
        if (isAdmin) await this.handleHapus(chatId, args, cfg);
        break;
      // Reseller commands
      case "/daftar":
        await this.handleDaftar(chatId, userId, username, cfg);
        break;
      case "/saldo":
        await this.handleSaldo(chatId, userId, cfg);
        break;
      case "/riwayat":
        await this.handleRiwayat(chatId, userId, cfg);
        break;
      // Tambah di switch(command)
      case "/topup":
        await this.handleTopupRequest(chatId, userId, username, args, cfg);
        break;
      case "/cektopup":
        await this.handleCekTopup(chatId, userId, cfg);
        break;
      case "/resellers":
        if (isAdmin) await this.handleListResellers(chatId, cfg);
        break;
      default:
        await this.sendMessage(
          chatId,
  ...
  private async handleCallback(cb: any, cfg: TelegramConfig): Promise<void> {
    ...
    switch (action) {
      ...
      case "gen_confirm":
        ...
        break;
      case "cancel":
        await this.cbCancel(chatId, msgId);
        break;
      case "gen_qty_custom":
        await this.cbAskCustomQty(chatId, userId, params[0], msgId, cfg);
        break;
      // Tambah di switch(action)
      case "topup_approve":
        await this.cbTopupApprove(chatId, userId, msgId, params[0], cfg);
        break;
      case "topup_reject":
        await this.cbTopupReject(chatId, userId, msgId, params[0], cfg);
        break;
      // Tambah di switch(action)
      case "topup_req":
        await this.cbTopupSelectNominal(
          chatId,
          userId,
          username,
          msgId,
        ...
        break;
      case "topup_custom":
        await this.cbTopupCustom(chatId, userId, msgId, cfg);
        break;
      // Di switch(action) di handleCallback
      case "topup_confirm":
        await this.cbTopupConfirm(
          chatId,
          userId,
          username,
          msgId,
        ...
        break;
      case "topup_back":
        await this.handleTopupRequest(chatId, userId, username, [], cfg);
        break;
      default:
        ...
        `Konfirmasi request?`,
      {
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "✅ Ya, Request Sekarang",
                callback_data: `topup_confirm:${amount}`
              },
  ...
  private async cbTopupApprove(
    ...
    if (!this.resellerSvc) {
      await this.editMessage(
        chatId,
        msgId,
        "❌ Layanan reseller tidak tersedia."
      );
      return;
    }
    ...
    if (!result) {
      await this.editMessage(
        chatId,
        msgId,
        "❌ Gagal memproses topup. Reseller tidak ditemukan."
      );
      return;
    }
    ...
    this.addLog(
      "ADMIN",
      `Approve topup ${req.resellerName} Rp ${req.amount.toLocaleString("id-ID")}`
    );
  }
  ...
  private async cbTopupReject(
    ...
    this.addLog(
      "ADMIN",
      `Reject topup ${req.resellerName} Rp ${req.amount.toLocaleString("id-ID")}`
    );
  }
  ...
  private async handleBeliMenu(
    ...
    if (!this.mikrotikService || !this.configService) {
      await this.sendMessage(chatId, "⚠️ Layanan belum siap. Coba lagi.");
      return;
    }
    ...
    try {
      ...
      if (!items.length) {
        ...
        let profiles: any[] = [];
        try {
          profiles = await client.run("/ip/hotspot/user/profile/print");
        } finally {
          client.close();
        }
  ...
  private async cbSelectProfile(
    ...
    if (!p) {
      await this.editMessage(
        chatId,
        msgId,
        "❌ Voucher tidak ditemukan. Coba lagi dengan /beli."
      );
    ...
    if (mode === "beli") {
              ...
              [
                {
                  ...
                  callback_data: `beli_confirm:${profileName}`
                },
                { text: "❌ Batal", callback_data: "cancel" }
              ]
            ]
      ...
      qtyRows.push(
        qtyOptions.map((q) => ({
          text: `${q}`,
          callback_data: `gen_qty:${profileName}:${q}`
        }))
      );
      // Custom qty button
      qtyRows.push([
        {
          text: "✏️ Jumlah lain (ketik angka)",
          callback_data: `gen_qty_custom:${profileName}`
        }
      ]);
  ...
  private async handleQtyInput(
    ...
    const ol = p
      ? this.parseOnLogin(p["on-login"] || "")
      : { sprice: 0, price: 0, validity: "" };
      ...
      `📦 Profile: <b>${state.profile}</b>\n📊 Jumlah: <b>${qty} voucher</b>\n💰 Total: <b>${fmtTotal}</b>\n\nKonfirmasi generate?`,
      {
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: `✅ Ya, Generate ${qty} Voucher`,
                callback_data: `gen_confirm:${state.profile}:${qty}`
              },
              { text: "❌ Batal", callback_data: "cancel" }
            ]
          ]
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: `✅ Ya, Generate ${qty} Voucher`,
                callback_data: `gen_confirm:${profileName}:${qty}`
              },
  ...
  private async cbConfirmBeli(
    ...
    await this.editMessage(
      chatId,
      msgId,
      `⏳ Membuat voucher <b>${profileName}</b>...`
    );
  ...
  private async cbConfirmGenerate(
    ...
    await this.editMessage(
      chatId,
      msgId,
      `⏳ Membuat <b>${qty}</b> voucher <b>${profileName}</b>...`
    );
    ...
    try {
      ...
      try {
        ...
        if (!profiles[0]) {
          ...
          editMsgId
            ? await this.editMessage(chatId, editMsgId, msg)
        ...
        const text = [
          ...
          profiles[0]["rate-limit"]
            ? `📶 Kecepatan: ${profiles[0]["rate-limit"]}`
            : "",
          ``,
          `🕐 ${new Date().toLocaleString("id-ID")}`
        ]
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: "🎟️ Beli Lagi",
                    callback_data: `beli_prof:${profileName}`
                  }
        ...
        if (this.resellerSvc) {
          ...
          if (reseller && reseller.status === "active") {
            ...
            if (reseller.saldo >= realprice) {
              ...
              try {
                const usrs = await client.run("/ip/hotspot/user/print", {
                  "?name": uname
                });
                if (usrs[0]?.[".id"])
                  await client.run("/ip/hotspot/user/remove", {
                    ".id": usrs[0][".id"]
                  });
              } catch {}
    ...
    try {
      ...
      try {
        ...
        if (!profiles[0]) {
          await this.sendMessage(
            chatId,
            `❌ Profile <b>${profileName}</b> tidak ditemukan.`
          );
          return;
        }
        const ol = this.parseOnLogin(profiles[0]["on-login"] || "");
        ...
        for (let i = 0; i < vouchers.length; i += chunkSize) {
          ...
          text += `</pre>`;
          await this.sendMessage(chatId, text);
          if (i + chunkSize < vouchers.length)
  ...
  private async handleProfil(
    ...
    try {
      ...
      try {
        ...
        if (!hasPrice)
          text += "Belum ada profile dengan harga.\nHubungi admin.";
        else text += `\nKetik /beli untuk membeli voucher.`;
        await this.sendMessage(chatId, text);
      } finally {
        client.close();
  ...
  private async handleCek(
    ...
    try {
      ...
      try {
        ...
        if (u["limit-uptime"])
          text += `⏰ Limit Uptime: ${u["limit-uptime"]}\n`;
        await this.sendMessage(chatId, text);
      } finally {
        client.close();
      }
  ...
  private async handleCekTopup(
    ...
    const requests = this.loadTopupRequests();
    const myReqs = requests.filter((r) => r.telegramId === userId).slice(0, 5);
  ...
  private async handleAktif(
    ...
    try {
      ...
      if (!active.length) {
        await this.sendMessage(chatId, "📭 Tidak ada user hotspot aktif.");
        return;
      }
  ...
  private async handleRekap(
    ...
    try {
      ...
      try {
        const scripts = await client.run("/system/script/print", {
          "?owner": idbl
        });
        const filtered =
          period === "today"
            ? scripts.filter(
                (r: any) => (r.name || "").split("-|-")[0] === idhr
              )
            : scripts;
        ...
        const rsList = Object.entries(resellerMap).sort(
  ...
  private async handlePppoe(
    ...
    try {
      ...
      if (!active.length) {
        await this.sendMessage(chatId, "📭 Tidak ada koneksi PPPoE aktif.");
        return;
      }
  ...
  private async handleHapus(
    ...
    const username = args[0];
    if (!username) {
      await this.sendMessage(chatId, "❓ Gunakan: /hapus [username]");
      return;
    }
    try {
      const s = this.configService.getDecryptedSession(cfg.sessionId);
      ...
      try {
        ...
        await this.sendMessage(
          chatId,
          `✅ User <code>${username}</code> berhasil dihapus.`
        );
  ...
  private scheduleDailyReport(configId: string): void {
    ...
    state.dailyTimer = setTimeout(async () => {
      ...
      this.scheduleDailyReport(configId);
    }, delay);
